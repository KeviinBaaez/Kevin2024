...
namespace Kevin2024.Windows.Formularios
{
    public partial class frmTiposTelefonos : Form
    {
        ...
        //Tipo para busqueda
        Archivo archivo = Archivo.Telefono;

        public frmTiposTelefonos(IServiceProvider? serviceProvider)
        {
            InitializeComponent();
        ...
        private void tsbNuevo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                TiposDeDatos? telefono = frm.GetTipoDeDato();
                if (telefono is null) return;
                if (!_servicios!.Existe(archivo, telefono))
                {
                    _servicios!.Guardar(archivo, telefono);
                    totalRecords = _servicios.GetCantidad(archivo, filter);
                    totalPages = (int)Math.Ceiling((decimal)totalRecords / pageSize);
                    currentPage = _servicios.GetPaginaPorRegistro(archivo, telefono.Descripcion, pageSize);
                    LoadData(filter);
        ...
        private void tsbEditar_Click(object sender, EventArgs e)
        {
            ...
            TiposDeDatos? telefono = (TiposDeDatos)r.Tag;
            frmNuevoTipoAE frm = new frmNuevoTipoAE(_serviceProvider, archivo) { Text = "Editar Marca" };
            frm.SetTipoDeDato(telefono);
            DialogResult dr = frm.ShowDialog(this);
            if (dr == DialogResult.Cancel) return;
            telefono = frm.GetTipoDeDato();
            ...
            try
            {
                ...
                {
                    _servicios.Guardar(archivo, telefono);
                    GridHelper.SetearFila(r, telefono);
        ...
        private void tsbBorrar_Click(object sender, EventArgs e)
        {
            ...
            TiposDeDatos telefono = (TiposDeDatos)r.Tag;
            DialogResult dr = MessageBox.Show($"¿Deseas dar de baja el archivo de teléfono: {telefono.Descripcion}?",
                "Confirmar",
            ...
            try
            {
                if (!_servicios!.EstaRelacionado(archivo, telefono.TipoId))
                {
                    _servicios!.Borrar(archivo, telefono.TipoId);
                    totalRecords = _servicios!.GetCantidad(archivo, filter);
                    totalPages = (int)Math.Ceiling((decimal)totalRecords / pageSize);
